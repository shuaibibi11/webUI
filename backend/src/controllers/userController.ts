...
export const register = async (req: Request, res: Response) => {
  try {
    ...
    const existingRealId = await prisma.user.findFirst({ where: { realName, idCard } });
    if (existingRealId) {
...
export const loginValidation = [
  body('username')
    ...
    .custom((value) => {
      ...
      if (isEmail) return true; // 邮箱格式在登录时放宽，由业务逻辑再校验
...
export const login = async (req: Request, res: Response) => {
  try {
    ...
    // 写入登录审计日志
    try {
      await prisma.auditLog.create({ userId: user.id, action: 'login', ip: (req.headers['x-forwarded-for'] as string) || req.ip, details: '用户登录' });
    } catch {}
